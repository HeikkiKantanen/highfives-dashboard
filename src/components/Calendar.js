...
export default function Calendar() {
	...
	let sortByMonth = (startDate, endDate) => {
		...
		sortIndexes.indexStart = arr.findIndex(
			(item) => item.date.yy === startDate.yy && item.date.mm === startDate.mm
		);
		for (let i = 0; i < arr.length; i++) {
			if (arr[i].date.yy === endDate.yy && arr[i].date.mm === endDate.mm)
				sortIndexes.indexEnd = arr.indexOf(arr[i]);
		}
		return arr.slice(sortIndexes.indexStart, sortIndexes.indexEnd + 1);
	};

	console.log(
		sortByMonth({ yy: 2022, mm: 1, dd: 2 }, { yy: 2022, mm: 3, dd: 2 })
	);
